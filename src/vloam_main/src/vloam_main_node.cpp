...
int main(int argc, char** argv)
{
  ...
  sub_image00_ptr =
      std::make_shared<message_filters::Subscriber<sensor_msgs::Image>>(nh, "/kitti/camera_color_left/image_raw", 100);
  sub_camera00_ptr = std::make_shared<message_filters::Subscriber<sensor_msgs::CameraInfo>>(
      nh, "/kitti/camera_color_left/camera_info", 100);
  sub_velodyne_ptr = std::make_shared<message_filters::Subscriber<sensor_msgs::PointCloud2>>(
      nh, "/kitti/velo/pointcloud", 100);  // TODO: change the buffer size to potentially reduce the data lost
