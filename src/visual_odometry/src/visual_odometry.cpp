...
void VisualOdometry::init(std::shared_ptr<VloamTF>& vloam_tf_)
{
  ...
  image_util.detector_type = DetectorType::SuperPoint; //ShiTomasi SuperPoint
  
  image_util.descriptor_type = DescriptorType::SuperPoint; //ORB SuperPoint
  image_util.matcher_type = MatcherType::SuperGlue; //BF  SuperGlue
...
void VisualOdometry::reset()
{
  ++count;
  i = count % 2;
  ROS_INFO("Count in VO is  number = %ld \n", count);
  ...
  if (count > 0)
  {
    if (!optical_flow_match)
      matches = image_util.matchDescriptors(descriptors[1 - i],
                                            descriptors[i],images[1-i],images[i],keypoints[1-i],keypoints[i]);  // first one is prev image, second one is curr image
    else
